    ...
    "Statement": [
        {
            "Effect": "Allow",
            "Principal": {
                "Federated": "arn:aws:iam::${data.aws_caller_identity.current.account_id}:oidc-provider/${var.oidc-url}"
            },
...
resource "aws_iam_role_policy_attachment" "eks-alb-attach" {
...
resource "aws_security_group_rule" "alb-tcp-9443" {
  description     =  "Allow EKS to connect to nodes for ALB Ingress"
  type              = "ingress"
  from_port           = 9443
  to_port             = 9443
  protocol          = "tcp"
